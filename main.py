...
def experiment_main(cfg=None):
    ...
    if cfg.model == "MLP":
        ...
        torchinfo.summary(model, input_size=(cfg.minibatch_size, cfg.n_hid_chan, (cfg.width + 2), (cfg.width + 2)))
...
def render_trained(ca, maze_data, cfg, pyplot_animation=True):
    ...
    if pyplot_animation:
        ...
        imgs = get_imgs(x, x0)
        pl_im = plt.imshow(imgs, interpolation='none')
        ...
        fig, animate, init_func=init, interval=1, frames=cfg.n_layers, blit=True, save_count=50)
